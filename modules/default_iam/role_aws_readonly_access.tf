...
resource "aws_iam_role" "aws_readonly_access" {
  count                = contains(var.roles, "aws_readonly_access") ? 1 : 0
  name                 = "aws_readonly_access"
  assume_role_policy   = data.aws_iam_policy_document.saml_federated_and_api_access_assume.json
  max_session_duration = var.max_session_duration
}

resource "aws_iam_role_policy_attachment" "aws_readonly_access" {
  count      = contains(var.roles, "aws_readonly_access") ? 1 : 0
  role       = aws_iam_role.aws_readonly_access[count.index].name
...
resource "aws_iam_policy" "restrictions_readonly" {
  count       = contains(var.roles, "aws_readonly_access") ? 1 : 0
  name        = "restrictions_readonly_policy"
  path        = "/"
